...
int ocheckusername(struct soap* soap)
{
	...
	{
		if (strcmp(oSysInfo->acounts[i].user,
		           soap->header->wsse__Security->UsernameToken->Username) == 0)
		{
...
char* oget_ipaddress(void)
{
	...
	sprintf(_IPv4Address, "http://%03d.%03d.%03d.%03d", ip.str[0], ip.str[1],
	        ip.str[2], ip.str[3]);
	return _IPv4Address;
#endif
	return NULL;
...
int oget_encoding(void)
{
	...
	else if(encode == 2)
	{
		if((oSysInfo->lan_config.nVideocodecres == 0) ||
		        ...
		        (oSysInfo->lan_config.nVideocodecres == 4))   //H264
		{
			encode = 2;
		}
		else
...
int oget_framerate(void)
{
	...
	if((oSysInfo->lan_config.nVideocombo == 0) ||
	        (oSysInfo->lan_config.nVideocombo == 1))   // H264 or MPEG4
	{
		if((oSysInfo->lan_config.nVideocodecres == 0) ||
		        (oSysInfo->lan_config.nVideocodecres == 1) ||
		        (oSysInfo->lan_config.nVideocodecres == 2))   //720/D1/SXVGA
		{
			if(oSysInfo->lan_config.nFrameRate1 == 0)
			{
				return 30;
			}
			else if(oSysInfo->lan_config.nFrameRate1 == 1)
			{
				return 24;
			}
			else if(oSysInfo->lan_config.nFrameRate1 == 2)
			{
				return 15;
			}
			else
	...
	{
		if((oSysInfo->lan_config.nVideocodecres == 0) ||
		        (oSysInfo->lan_config.nVideocodecres == 1))   // 2MP
		{
			if(oSysInfo->lan_config.nFrameRate1 == 0)
			{
				return 30;
			...
			else
			{
		        ...
		        (oSysInfo->lan_config.nVideocodecres == 3))   // 3MP
		{
			if(oSysInfo->lan_config.nFrameRate1 == 1)
			{
				return 15;
			}
			else
...
int oget_resolutionwidth(void)
{
	...
	if(oSysInfo->lan_config.nVideocombo ==
	        0)   //--------------- 264 --------------//
	{
	...
	else if (oSysInfo->lan_config.nVideocombo ==
	         1)   //--------------- MPEG4 --------------//
	{
	...
	{
		if((oSysInfo->lan_config.nVideocodecres == 0) ||
		        (oSysInfo->lan_config.nVideocodecres == 1))  	// JPG:1600X1200
		{
			return 1600;	//return 1200;
		}
		else if((oSysInfo->lan_config.nVideocodecres == 2) ||
		        (oSysInfo->lan_config.nVideocodecres == 3))   // JPG:2048X1536
		{
			return 2048;	//return 1536;
		}
		else if((oSysInfo->lan_config.nVideocodecres == 4) ||
		        (oSysInfo->lan_config.nVideocodecres == 5))   // JPG:2048X1536
		{
...
int oset_framerate(int encoding, int resolution, int framerate)
{
	...
	{
		ControlSystemData(SFIELD_SET_FRAMERATE1, (void*)&_Framerate,
		                  sizeof(_Framerate));
	}
...
unsigned char  oset_resolution(unsigned char encoding, int width, int height)
{
	...
	if(encoding == 0)
	{
		encoding = 2;    // jpeg  -> Megapixel
	}
	else if(encoding == 1)
	{
		encoding = 1;    // MPEG4 -> MPEG4
	}
	else if(encoding == 2)
	{
		encoding = 0;    // H264  -> H264
	}
...
int oset_timezone(char* TZ)
{
	...
	else if ((strncmp(TZ, "AHST", 4) == 0) || (strncmp(TZ, "CAT", 3) == 0) ||
	         (strncmp(TZ, "HST", 3) == 0) || (strncmp(TZ, "HDT", 3) == 0))
	{
		return 2;    //-10
	}
	else if ((strncmp(TZ, "YST", 3) == 0) || (strncmp(TZ, "YDT", 3) == 0))
	{
		return 3;    //-9
	}
	else if ((strncmp(TZ, "PST", 3) == 0) || (strncmp(TZ, "PDT", 3) == 0))
	{
		return 4;    //-8
	}
	else if ((strncmp(TZ, "MST", 3) == 0) || (strncmp(TZ, "MDT", 3) == 0))
	{
		return 5;    //-7
	}
	else if ((strncmp(TZ, "CST", 3) == 0) || (strncmp(TZ, "CDT", 3) == 0))
	{
		return 6;    //-6
	}
	else if ((strncmp(TZ, "EST", 3) == 0) || (strncmp(TZ, "EDT", 3) == 0))
	{
		return 7;    //-5
	}
	else if ((strncmp(TZ, "AST", 3) == 0) || (strncmp(TZ, "ADT", 3) == 0))
	{
		return 8;    //-4
	}
	...
	else if ((strncmp(TZ, "GMT", 3) == 0) || (strncmp(TZ, "UT", 2) == 0) ||
	         (strncmp(TZ, "UTC", 3) == 0) || (strncmp(TZ, "BST", 3) == 0))
	{
		return 12;    //-0
	}
	else if ((strncmp(TZ, "CET", 3) == 0) || (strncmp(TZ, "FWT", 3) == 0) ||
	         (strncmp(TZ, "MET", 3) == 0)\
	         || (strncmp(TZ, "MEWT", 4) == 0) || (strncmp(TZ, "SWT", 3) == 0) ||
	         (strncmp(TZ, "MEST", 4) == 0)\
	         || (strncmp(TZ, "MESZ", 4) == 0) || (strncmp(TZ, "SST", 3) == 0) ||
	         (strncmp(TZ, "FST", 3) == 0))
	{
		return 13;    //1
	}
