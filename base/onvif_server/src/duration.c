...
int soap_out_xsd__duration(struct soap* soap, const char* tag, int id, const LONG64* a,
                           const char* type)
{
...
int soap_s2xsd__duration(struct soap* soap, const char* s, LONG64* a)
{
	...
	*a = 0;
	...
	if (s)
	{
		...
		*a = sign * ((((((((((((Y * 12) + M) * 30) + D) * 24) + H) * 60) + N) * 60) + S) * 1000) + (long)(
		                 1000.0 * f));
	}
...
LONG64* soap_in_xsd__duration(struct soap* soap, const char* tag, LONG64* a, const char* type)
{
	...
	a = (LONG64*)soap_id_enter(soap, soap->id, a, SOAP_TYPE_xsd__duration, sizeof(LONG64), 0, NULL,
	                           NULL, NULL);
