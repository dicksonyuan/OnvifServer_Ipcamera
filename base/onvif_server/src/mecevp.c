...
static int soap_mec_upd(struct soap* soap, struct soap_mec_data* data, const char** s, size_t* n,
                        int final);
static int soap_mec_upd_enc(struct soap* soap, struct soap_mec_data* data, const char** s,
                            size_t* n, int final);
static int soap_mec_upd_dec(struct soap* soap, struct soap_mec_data* data, const char** s,
                            size_t* n, int final);

static int soap_mec_check(struct soap* soap, struct soap_mec_data* data, int err, const char* msg);

static void soap_mec_put_base64(struct soap* soap, struct soap_mec_data* data,
                                const unsigned char* s, int n);
static void soap_mec_end_base64(struct soap* soap, struct soap_mec_data* data);
static int soap_mec_get_base64(struct soap* soap, struct soap_mec_data* data, char* t, size_t* l,
                               const char* s, size_t n, const char** r, size_t* k);
...
*/
int
soap_mec_init(struct soap* soap, struct soap_mec_data* data, int alg, SOAP_MEC_KEY_TYPE* pkey,
...
*/
int
soap_mec_begin(struct soap* soap, struct soap_mec_data* data, int alg, SOAP_MEC_KEY_TYPE* pkey,
               unsigned char* key, int* keylen)
{
...
soap_mec_upd(struct soap* soap, struct soap_mec_data* data, const char** s, size_t* n, int final)
{
	...
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "-- MEC Update alg=%x n=%lu final=%d (%p) --\n", data->alg,
	                          (unsigned long)*n, final, data->ctx));
	DBGMSG(TEST, *s, *n);
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "\n--\n"));
...
*/
static int
soap_mec_upd_enc(struct soap* soap, struct soap_mec_data* data, const char** s, size_t* n,
                 int final)
{
	size_t k;
	int m;
	...
	if (!final)
	{
		...
		{
			ok = EVP_SealUpdate(data->ctx, (unsigned char*)data->buf + data->buflen - k, &m, (unsigned char*)*s,
			                    (int) * n);
		}
		else
		{
			ok = EVP_EncryptUpdate(data->ctx, (unsigned char*)data->buf + data->buflen - k, &m,
...
*/
static int
soap_mec_upd_dec(struct soap* soap, struct soap_mec_data* data, const char** s, size_t* n,
                 int final)
{
	const char* r = NULL;
	size_t k = 0, l = 0, m = 0;
	...
	if (state == SOAP_MEC_STATE_INIT)
	{
		...
		if (soap_mec_get_base64(soap, data, data->buf + data->buflen - k, &m, data->buf, data->bufidx, &r,
		                        &l))
		{
			return soap->error;
		}
	...
	switch (data->state)
	{
		...
		case SOAP_MEC_STATE_IV:
			...
			if (data->bufidx >= (size_t)EVP_CIPHER_iv_length(data->type))
			{
				...
				switch (data->alg & SOAP_MEC_MASK)
				{
					...
					case SOAP_MEC_ENV_DEC_AES256_CBC:
					case SOAP_MEC_ENV_DEC_AES512_CBC:
						ok = EVP_OpenInit(data->ctx, data->type, data->key, data->keylen, (unsigned char*)data->buf,
						                  (EVP_PKEY*)data->pkey);
				...
				if (ok)
				{
					...
					switch (data->alg & SOAP_MEC_MASK)
					{
						...
						case SOAP_MEC_ENV_DEC_AES256_CBC:
						case SOAP_MEC_ENV_DEC_AES512_CBC:
							ok = EVP_OpenUpdate(data->ctx, (unsigned char*)data->buf, &len,
							                    (unsigned char*)(data->buf + data->buflen - k), k);
							break;
						...
						case SOAP_MEC_DEC_AES256_CBC:
						case SOAP_MEC_DEC_AES512_CBC:
							ok = EVP_DecryptUpdate(data->ctx, (unsigned char*)data->buf, &len,
							                       (unsigned char*)(data->buf + data->buflen - k), k);
							break;
					}
		...
		case SOAP_MEC_STATE_DECRYPT:
			...
			switch (data->alg & SOAP_MEC_MASK)
			{
				...
				case SOAP_MEC_ENV_DEC_AES256_CBC:
				case SOAP_MEC_ENV_DEC_AES512_CBC:
					ok = EVP_OpenUpdate(data->ctx, (unsigned char*)(data->buf + data->bufidx), &len,
					                    (unsigned char*)(data->buf + data->buflen - k), m);
					break;
				...
				case SOAP_MEC_DEC_AES256_CBC:
				case SOAP_MEC_DEC_AES512_CBC:
					ok = EVP_DecryptUpdate(data->ctx, (unsigned char*)(data->buf + data->bufidx), &len,
					                       (unsigned char*)data->buf + data->buflen - k, m);
					break;
			}
...
*/
static int
soap_mec_get_base64(struct soap* soap, struct soap_mec_data* data, char* t, size_t* l,
                    const char* s, size_t n, const char** r, size_t* k)
{
	int i;
	unsigned long m;
