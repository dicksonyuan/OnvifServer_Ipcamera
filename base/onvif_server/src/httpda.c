...
static int http_da_session_update(const char* realm, const char* nonce, const char* opaque,
...
static void http_da_calc_HA1(struct soap* soap, void** context, const char* alg, const char* userid,
                             const char* realm, const char* passwd, const char* nonce, const char* cnonce, char HA1hex[33]);
static void http_da_calc_response(struct soap* soap, void** context, char HA1hex[33],
                                  const char* nonce, const char* ncount, const char* cnonce, const char* qop, const char* method,
                                  const char* uri, char entityHAhex[33], char response[33]);
...
static int http_da_copy(struct soap* soap, struct soap_plugin* dst, struct soap_plugin* src)
{
	...
	memset(((struct http_da_data*)dst->data)->digest, 0,
	       sizeof(((struct http_da_data*)dst->data)->digest));
...
static int http_da_post_header(struct soap* soap, const char* key, const char* val)
{
	...
	if (key && (!strcmp(key, "Authorization") || !strcmp(key, "Proxy-Authorization")))
	{
		...
		http_da_calc_HA1(soap, &data->context, data->alg, userid, soap->authrealm, passwd, data->nonce,
		                 cnonce, HA1);

		if (soap->status != SOAP_GET && soap->status != SOAP_CONNECT && data->qop &&
		        !soap_tag_cmp(data->qop, "*auth-int*"))
		{
			qop = "auth-int";
...
#endif
		http_da_calc_response(soap, &data->context, HA1, data->nonce, ncount, cnonce, qop, method,
		                      soap->path, entityHAhex, response);
#ifdef HAVE_SNPRINTF
		soap_snprintf(soap->tmpbuf, sizeof(soap->tmpbuf),
		              "Digest realm=\"%s\", username=\"%s\", nonce=\"%s\", uri=\"%s\", nc=%s, cnonce=\"%s\", response=\"%s\"",
		              soap->authrealm, userid, data->nonce, soap->path, ncount, cnonce, response);
#else
		sprintf(soap->tmpbuf,
		        "Digest realm=\"%s\", username=\"%s\", nonce=\"%s\", uri=\"%s\", nc=%s, cnonce=\"%s\", response=\"%s\"",
		        soap->authrealm, userid, data->nonce, soap->path, ncount, cnonce, response);
...
#ifdef HAVE_SNPRINTF
		soap_snprintf(soap->tmpbuf, sizeof(soap->tmpbuf),
		              "Digest realm=\"%s\", qop=\"auth,auth-int\", nonce=\"%s\", opaque=\"%s\"", soap->authrealm, nonce,
		              opaque);
#else
		sprintf(soap->tmpbuf, "Digest realm=\"%s\", qop=\"auth,auth-int\", nonce=\"%s\", opaque=\"%s\"",
...
static int http_da_parse_header(struct soap* soap, const char* key, const char* val)
{
	...
	if ((!soap_tag_cmp(key, "WWW-Authenticate") || !soap_tag_cmp(key, "Proxy-Authenticate")) &&
	        !soap_tag_cmp(val, "Digest *"))
	{
		soap->authrealm = soap_strdup(soap, soap_get_header_attribute(soap, val + 7, "realm"));
		data->nonce = soap_strdup(soap, soap_get_header_attribute(soap, val + 7, "nonce"));
		data->opaque = soap_strdup(soap, soap_get_header_attribute(soap, val + 7, "opaque"));
...
void http_da_save(struct soap* soap, struct http_da_info* info, const char* realm,
...
void http_da_proxy_save(struct soap* soap, struct http_da_info* info, const char* realm,
                        const char* userid, const char* passwd)
{
	struct http_da_data* data = (struct http_da_data*)soap_lookup_plugin(soap, http_da_id);
...
static int http_da_verify_method(struct soap* soap, const char* method, const char* passwd)
{
	...
	http_da_calc_HA1(soap, &data->context, NULL, soap->userid, soap->authrealm, passwd, data->nonce,
	                 data->cnonce, HA1);
	...
	http_da_calc_response(soap, &data->context, HA1, data->nonce, data->ncount, data->cnonce, data->qop,
	                      method, soap->path, entityHAhex, response);
#ifdef SOAP_DEBUG
