...
static int soap_wsse_session_verify(struct soap* soap, const char hash[SOAP_SMD_SHA1_SIZE],
                                    const char* created, const char* nonce);
static void soap_wsse_session_cleanup(struct soap* soap);
static void calc_digest(struct soap* soap, const char* created, const char* nonce, int noncelen,
                        const char* password, char hash[SOAP_SMD_SHA1_SIZE]);
...
static int soap_wsse_init(struct soap* soap, struct soap_wsse_data* data,
                          const void* (*arg)(struct soap*, int*, const char*, int*));
...
static size_t soap_wsse_verify_nested(struct soap* soap, struct soap_dom_element* dom,
                                      const char* URI, const char* tag);
...
soap_wsse_add_Security_actor(struct soap* soap, const char* actor)
{
	...
	if (soap->namespaces &&
	        !strcmp(soap->namespaces[0].ns, "http://schemas.xmlsoap.org/soap/envelope/"))
	{
...
*/
int
soap_wsse_add_UsernameTokenText(struct soap* soap, const char* id, const char* username,
                                const char* password)
{
	_wsse__Security* security = soap_wsse_add_Security(soap);
	DBGFUN2("soap_wsse_add_UsernameTokenText", "id=%s", id ? id : "", "username=%s",
	        username ? username : "");

	/* allocate a UsernameToken if we don't have one already */
	if (!security->UsernameToken)
	{
		if (!(security->UsernameToken = (_wsse__UsernameToken*)soap_malloc(soap,
		                                sizeof(_wsse__UsernameToken))))
		{
			return soap->error = SOAP_EOM;
		}
	}
	...
	if (password)
	{
		if (!(security->UsernameToken->Password = (_wsse__Password*)soap_malloc(soap,
		        sizeof(_wsse__Password))))
		{
			return soap->error = SOAP_EOM;
		}
...
*/
int
soap_wsse_add_UsernameTokenDigest(struct soap* soap, const char* id, const char* username,
                                  const char* password)
{
	_wsse__Security* security = soap_wsse_add_Security(soap);
	time_t now = time(NULL);
	...
	DBGFUN2("soap_wsse_add_UsernameTokenDigest", "id=%s", id ? id : "", "username=%s",
	        username ? username : "");
	/* generate a nonce */
...
*/
int
soap_wsse_add_BinarySecurityToken(struct soap* soap, const char* id, const char* valueType,
                                  const unsigned char* data, int size)
{
	_wsse__Security* security = soap_wsse_add_Security(soap);
	DBGFUN2("wsse_add_BinarySecurityToken", "id=%s", id ? id : "", "valueType=%s",
	        valueType ? valueType : "");

	/* allocate BinarySecurityToken if we don't already have one */
	if (!security->BinarySecurityToken)
	{
		if (!(security->BinarySecurityToken = (_wsse__BinarySecurityToken*)soap_malloc(soap,
		                                      sizeof(_wsse__BinarySecurityToken))))
		{
			return soap->error = SOAP_EOM;
		}
	}
...
{
	FILE* fd;
	DBGFUN2("soap_wsse_add_BinarySecurityTokenPEM", "id=%s", id ? id : "", "filename=%s",
	        filename ? filename : "");
...
*/
int
soap_wsse_get_BinarySecurityToken(struct soap* soap, const char* id, char** valueType,
...
soap_wsse_verify_X509(struct soap* soap, X509* cert)
{
	...
	if (!data->store)
	{
		...
		{
			return soap_wsse_receiver_fault(soap, "soap_wsse_verify_X509",
			                                "Could not create X509_STORE object");
		}
		...
		if (soap->crlfile)
		{
			...
			{
				return soap_wsse_receiver_fault(soap, "soap_wsse_verify_X509",
				                                "Could not create X509_LOOKUP object");
			}
	...
	{
		return soap_wsse_receiver_fault(soap, "soap_wsse_verify_X509",
		                                "Could not create X509_STORE_CTX object");
	}
	...
	if (X509_STORE_CTX_init(verify, data->store, cert, NULL) != 1)
	{
		...
		return soap_wsse_receiver_fault(soap, "soap_wsse_verify_X509",
		                                "Could not initialize X509_STORE_CTX object");
	}
...
soap_wsse_add_SecurityContextToken(struct soap* soap, const char* id, const char* identifier)
{
	char* URI;
	_wsse__Security* security = soap_wsse_add_Security(soap);
	DBGFUN2("soap_wsse_add_SecurityContextToken", "id=%s", id, "identifier=%s",
	        identifier ? identifier : "");

	/* allocate wsc:SecurityContextToken if we don't already have one */
	if (!security->wsc__SecurityContextToken)
	{
		if (!(security->wsc__SecurityContextToken = (struct wsc__SecurityContextTokenType*)soap_malloc(soap,
		        sizeof(struct wsc__SecurityContextTokenType))))
		{
			return soap->error = SOAP_EOM;
		}
	}
...
*/
int
soap_wsse_add_SignedInfo_Reference(struct soap* soap, const char* URI, const char* transform,
                                   const char* inclusiveNamespaces, int alg, const char* HA)
{
	ds__SignedInfoType* signedInfo = soap_wsse_add_SignedInfo(soap);
	ds__ReferenceType* reference;
	DBGFUN3("soap_wsse_add_SignedInfo_Reference", "URI=%s", URI ? URI : "", "transform=%s",
	        transform ? transform : "", "alg=%x", alg);

	/* if this is the first reference, allocate SOAP_WSSE_MAX_REF references */
	if (signedInfo->__sizeReference == 0)
	{
		signedInfo->Reference = (ds__ReferenceType**)soap_malloc(soap,
		                        SOAP_WSSE_MAX_REF * sizeof(ds__ReferenceType*));
	}
	else
	{
	...
	if (transform)
	{
		...
		{
			reference->Transforms->Transform->c14n__InclusiveNamespaces = (_c14n__InclusiveNamespaces*)
			        soap_malloc(soap, sizeof(_c14n__InclusiveNamespaces));
			...
			soap_default__c14n__InclusiveNamespaces(soap,
			                                        reference->Transforms->Transform->c14n__InclusiveNamespaces);
			reference->Transforms->Transform->c14n__InclusiveNamespaces->PrefixList = soap_strdup(soap,
			        inclusiveNamespaces);
		}
	}
...
{
	ds__SignedInfoType* signedInfo = soap_wsse_add_SignedInfo(soap);
	DBGFUN2("soap_wsse_add_SignedInfo_SignatureMethod", "method=%s", method ? method : "",
	        "canonical=%d", canonical);
	/* if signed in exc-c14n form, populate CanonicalizationMethod element */
	signedInfo->CanonicalizationMethod = (ds__CanonicalizationMethodType*)soap_malloc(soap,
	                                     sizeof(ds__CanonicalizationMethodType));
	...
	signedInfo->SignatureMethod = (ds__SignatureMethodType*)soap_malloc(soap,
	                              sizeof(ds__SignatureMethodType));
...
soap_wsse_verify_SignatureValue(struct soap* soap, int alg, const void* key, int keylen)
{
	        ...
	        && signature->SignatureValue)
	{
		...
		else
		{
			...
			if (soap_smd_end(soap, sig, &siglen) || err)
			{
				return soap_wsse_fault(soap, wsse__FailedCheck,
				                       "The signed serialized SignedInfo SignatureValue is invalid");
			}
			...
			if ((alg & SOAP_SMD_ALGO) == SOAP_SMD_HMAC)
			{
				...
				{
					return soap_wsse_fault(soap, wsse__FailedCheck,
					                       "The HMAC-signed serialized SignedInfo is invalid");
				}
			}
		...
		for (i = 0; i < signedInfo->__sizeReference; i++)
		{
			...
			if (*reference->URI == '#')
			{
				...
				if (reference->Transforms)
				{
					...
					{
						return soap_wsse_fault(soap, wsse__UnsupportedAlgorithm,
						                       reference->Transforms->Transform[0].Algorithm);
					}
...
*/
int
soap_wsse_verify_digest(struct soap* soap, int alg, int canonical, const char* id,
                        unsigned char hash[SOAP_SMD_MAX_SIZE])
{
	struct soap_wsse_data* data = (struct soap_wsse_data*)soap_lookup_plugin(soap, soap_wsse_id);
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_verify_digest", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
...
*/
int
soap_wsse_add_KeyInfo_SecurityTokenReferenceURI(struct soap* soap, const char* URI,
        const char* valueType)
{
	ds__KeyInfoType* keyInfo = soap_wsse_add_KeyInfo(soap);
	DBGFUN2("soap_wsse_add_KeyInfo_SecurityTokenReferenceURI", "URI=%s", URI ? URI : "", "valueType=%s",
	        valueType ? valueType : "");

	/* allocate SecurityTokenReference element if we don't have one already */
	if (!keyInfo->wsse__SecurityTokenReference)
	{
		keyInfo->wsse__SecurityTokenReference = (_wsse__SecurityTokenReference*)soap_malloc(soap,
		                                        sizeof(_wsse__SecurityTokenReference));
	...
	keyInfo->wsse__SecurityTokenReference->Reference = (_wsse__Reference*)soap_malloc(soap,
	        sizeof(_wsse__Reference));
...
*/
int
soap_wsse_add_KeyInfo_SecurityTokenReferenceKeyIdentifier(struct soap* soap, const char* id,
        const char* valueType, unsigned char* data, int size)
{
	ds__KeyInfoType* keyInfo = soap_wsse_add_KeyInfo(soap);
	DBGFUN2("soap_wsse_add_KeyInfo_SecurityTokenReferenceKeyIdentifier", "id=%s", id ? id : "",
	        "valueType=%s", valueType ? valueType : "");
	...
	keyInfo->wsse__SecurityTokenReference->KeyIdentifier = (_wsse__KeyIdentifier*)soap_malloc(soap,
	        sizeof(_wsse__KeyIdentifier));
	...
	keyInfo->wsse__SecurityTokenReference->KeyIdentifier->__item = soap_s2base64(soap, data, NULL,
	        size);
	return SOAP_OK;
}
...
soap_wsse_get_KeyInfo_SecurityTokenReferenceKeyIdentifier(struct soap* soap, int* size)
{
	...
	return (unsigned char*)soap_base642s(soap,
	                                     keyInfo->wsse__SecurityTokenReference->KeyIdentifier->__item, NULL, 0, size);
}
...
*/
int
soap_wsse_add_KeyInfo_SecurityTokenReferenceEmbedded(struct soap* soap, const char* id,
        const char* valueType)
{
	ds__KeyInfoType* keyInfo = soap_wsse_add_KeyInfo(soap);
	...
	keyInfo->wsse__SecurityTokenReference->Embedded = (_wsse__Embedded*)soap_malloc(soap,
	        sizeof(_wsse__Embedded));
...
*/
int
soap_wsse_add_EncryptedKey(struct soap* soap, int alg, const char* URI, X509* cert,
                           const char* subjectkeyid, const char* issuer, const char* serial)
{
	return soap_wsse_add_EncryptedKey_encrypt_only(soap, alg, URI, cert, subjectkeyid, issuer, serial,
	        NULL);
}
...
*/
int
soap_wsse_add_EncryptedKey_encrypt_only(struct soap* soap, int alg, const char* URI, X509* cert,
                                        const char* subjectkeyid, const char* issuer, const char* serial, const char* tags)
{
	EVP_PKEY* pubk;
	unsigned char* key;
	int keylen;
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_add_EncryptedKey", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
	...
	{
		if (!(security->xenc__EncryptedKey = (xenc__EncryptedKeyType*)soap_malloc(soap,
		                                     sizeof(xenc__EncryptedKeyType))))
		{
			return soap->error = SOAP_EOM;
		}
	}
	...
	if (!(security->xenc__EncryptedKey->EncryptionMethod = (xenc__EncryptionMethodType*)soap_malloc(
	            soap, sizeof(struct xenc__EncryptionMethodType))))
	{
		return soap->error = SOAP_EOM;
	}
	...
	if (alg & SOAP_MEC_OAEP)
	{
		...
		if (!(security->xenc__EncryptedKey->EncryptionMethod->ds__DigestMethod =
		            (struct ds__DigestMethodType*)soap_malloc(soap, sizeof(struct ds__DigestMethodType))))
		{
			return soap->error = SOAP_EOM;
		}

		soap_default_ds__DigestMethodType(soap,
		                                  security->xenc__EncryptedKey->EncryptionMethod->ds__DigestMethod);
	...
	/* KeyInfo */
	if (!(security->xenc__EncryptedKey->ds__KeyInfo = (_ds__KeyInfo*)soap_malloc(soap,
	        sizeof(_ds__KeyInfo))))
	{
		return soap->error = SOAP_EOM;
	}
	...
	if (!(security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference =
	            (_wsse__SecurityTokenReference*)soap_malloc(soap, sizeof(_wsse__SecurityTokenReference))))
	{
		return soap->error = SOAP_EOM;
	}

	soap_default__wsse__SecurityTokenReference(soap,
	        security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference);

	if (issuer && serial)
	{
		if (!(security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->ds__X509Data =
		            (struct ds__X509DataType*)soap_malloc(soap, sizeof(struct ds__X509DataType))))
		{
			return soap->error = SOAP_EOM;
		}

		soap_default_ds__X509DataType(soap,
		                              security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->ds__X509Data);

		if (!(security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->ds__X509Data->X509IssuerSerial
		        = (struct ds__X509IssuerSerialType*)soap_malloc(soap, sizeof(struct ds__X509IssuerSerialType))))
		{
			return soap->error = SOAP_EOM;
		}

		soap_default_ds__X509IssuerSerialType(soap,
		                                      security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->ds__X509Data->X509IssuerSerial);
		security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->ds__X509Data->X509IssuerSerial->X509IssuerName
		    = soap_strdup(soap, issuer);
		security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->ds__X509Data->X509IssuerSerial->X509SerialNumber
		    = soap_strdup(soap, serial);
	}
	else
	{
		/* allocate KeyIdentifier */
		if (!(security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier =
		            (_wsse__KeyIdentifier*)soap_malloc(soap, sizeof(_wsse__KeyIdentifier))))
		{
			return soap->error = SOAP_EOM;
		}

		soap_default__wsse__KeyIdentifier(soap,
		                                  security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier);
		/* populate KeyIdentifier */
		security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier->EncodingType
		    = (char*)wsse_Base64BinaryURI;

		if (subjectkeyid)
		{
			security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier->ValueType
			    = (char*)wsse_X509v3SubjectKeyIdentifierURI;
			security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier->__item =
			    soap_s2base64(soap, (unsigned char*)subjectkeyid, NULL, strlen(subjectkeyid));
		}
		else if (cert)
		{
			unsigned char* der, *s;
			...
			if (derlen < 0)
			{
				return soap_wsse_fault(soap, wsse__InvalidSecurityToken,
				                       "Invalid certificate passed to soap_wsse_add_EncryptedKey_encrypt_only");
			}
			...
			i2d_X509(cert, &s);
			security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier->__item =
			    soap_s2base64(soap, der, NULL, derlen);
		}
	}

	/* CipherData */
	if (!(security->xenc__EncryptedKey->CipherData = (xenc__CipherDataType*)soap_malloc(soap,
	        sizeof(struct xenc__CipherDataType))))
	{
		return soap->error = SOAP_EOM;
	}
	...
	if (!(security->xenc__EncryptedKey->CipherData->CipherValue = soap_s2base64(soap, key, NULL,
	        keylen)))
	{
		return soap->error = SOAP_EOM;
	}
...
soap_wsse_verify_EncryptedKey(struct soap* soap)
{
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_verify_EncryptedKey", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
	...
	if (security && security->xenc__EncryptedKey)
	{
		...
		if (security->xenc__EncryptedKey->ds__KeyInfo)
		{
			        ...
			        && security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier->__item)
			{
				            ...
				            wsse_X509v3URI))
				{
					...
					int derlen;
					der = (unsigned char*)soap_base642s(soap,
					                                    security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier->__item,
					                                    NULL, 0, &derlen);
					...
					if (!data->deco_key && data->security_token_handler)
					{
						...
						data->deco_key = data->security_token_handler(soap, &data->deco_alg,
						                 X509_NAME_oneline(X509_get_subject_name(cert), buf, sizeof(buf)), &keylen);
						data->deco_keylen = 0;
					}
					...
					if (cert)
					{
						X509_free(cert);
				         ...
				         !strcmp(security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier->ValueType,
				                 wsse_X509v3SubjectKeyIdentifierURI))
				{
					int subjectkeyidlen;
					const char* subjectkeyid = (char*)soap_base642s(soap,
					                           security->xenc__EncryptedKey->ds__KeyInfo->wsse__SecurityTokenReference->KeyIdentifier->__item,
					                           NULL, 0, &subjectkeyidlen);
		...
		/* start decryption */
		if (data->deco_key && security->xenc__EncryptedKey->CipherData &&
		        security->xenc__EncryptedKey->CipherData->CipherValue)
		{
			int keylen;
			unsigned char* key = (unsigned char*)soap_base642s(soap,
			                     security->xenc__EncryptedKey->CipherData->CipherValue, NULL, 0, &keylen);
			...
			if (soap_mec_begin(soap, data->mec, data->deco_alg, (SOAP_MEC_KEY_TYPE*)data->deco_key, key,
			                   &keylen))
			{
				return soap_wsse_fault(soap, wsse__FailedCheck, NULL);
			}
...
soap_wsse_add_EncryptedKey_DataReferenceURI(struct soap* soap, const char* URI)
{
	...
	{
		if (!(security->xenc__EncryptedKey->ReferenceList = (struct _xenc__ReferenceList*)soap_malloc(soap,
		        sizeof(struct _xenc__ReferenceList))))
		{
			return soap->error = SOAP_EOM;
		}
	...
	if (n)
	{
		struct __xenc__union_ReferenceList* tmp = (struct __xenc__union_ReferenceList*)soap_malloc(soap,
		        n * sizeof(struct __xenc__union_ReferenceList));
		int i;
	...
	if (!(ref->__union_ReferenceList[k].DataReference = (struct xenc__ReferenceType*)soap_malloc(soap,
	        sizeof(struct xenc__ReferenceType))))
	{
		return soap->error = SOAP_EOM;
	}
...
soap_wsse_add_DataReferenceURI(struct soap* soap, const char* URI)
{
	...
	{
		if (!(security->xenc__ReferenceList = (struct _xenc__ReferenceList*)soap_malloc(soap,
		                                      sizeof(struct _xenc__ReferenceList))))
		{
			return soap->error = SOAP_EOM;
		}
...
soap_wsse_add_EncryptedData_KeyInfo_KeyName(struct soap* soap, const char* keyname)
{
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_add_EncryptedData_KeyInfo_KeyName",
		                               "Plugin not registered", SOAP_PLUGIN_ERROR);
	}
...
*/
int
soap_wsse_sender_fault_subcode(struct soap* soap, const char* faultsubcode, const char* faultstring,
...
*/
int
soap_wsse_receiver_fault_subcode(struct soap* soap, const char* faultsubcode,
...
soap_wsse_fault(struct soap* soap, wsse__FaultcodeEnum fault, const char* detail)
{
	...
	switch (fault)
	{
		...
		case wsse__UnsupportedAlgorithm:
			return soap_wsse_sender_fault_subcode(soap, code,
			                                      "An unsupported signature or encryption algorithm was used", detail);

		case wsse__InvalidSecurity:
			return soap_wsse_sender_fault_subcode(soap, code,
			                                      "An error was discovered processing the <wsse:Security> header", detail);
		...
		case wsse__FailedAuthentication:
			return soap_wsse_sender_fault_subcode(soap, code,
			                                      "The security token could not be authenticated or authorized", detail);

		case wsse__FailedCheck:
			return soap_wsse_sender_fault_subcode(soap, code, "The signature or decryption was invalid",
			                                      detail);

		case wsse__SecurityTokenUnavailable:
			return soap_wsse_sender_fault_subcode(soap, code,
			                                      "Referenced security token could not be retrieved", detail);
	}
...
*/
static int
soap_wsse_session_verify(struct soap* soap, const char hash[SOAP_SMD_SHA1_SIZE],
                         const char* created, const char* nonce)
{
...
*/
static void
calc_digest(struct soap* soap, const char* created, const char* nonce, int noncelen,
            const char* password, char hash[SOAP_SMD_SHA1_SIZE])
{
...
soap_wsse(struct soap* soap, struct soap_plugin* p, void* arg)
{
	...
	if (p->data)
	{
		if (soap_wsse_init(soap, (struct soap_wsse_data*)p->data, (const void* (*)(struct soap*, int*,
		                   const char*, int*))arg))
		{
...
*/
static int
soap_wsse_init(struct soap* soap, struct soap_wsse_data* data, const void* (*arg)(struct soap*,
               int* alg, const char* keyname, int* keylen))
{
...
soap_wsse_copy(struct soap* soap, struct soap_plugin* dst, struct soap_plugin* src)
{
	...
	*dst = *src;
	...
	soap_wsse_init(soap, (struct soap_wsse_data*)dst->data,
	               ((struct soap_wsse_data*)src->data)->security_token_handler);
	return SOAP_OK;
}
...
soap_wsse_verify_auto(struct soap* soap, int alg, const void* key, size_t keylen)
{
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_verify_auto", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
...
soap_wsse_verify_done(struct soap* soap)
{
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_verify_done", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
...
*/
static size_t
soap_wsse_verify_nested(struct soap* soap, struct soap_dom_element* dom, const char* URI,
                        const char* tag)
{
	size_t count = 0;
...
soap_wsse_encrypt_body(struct soap* soap, int alg, const void* key, int keylen)
{
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_encrypt_body", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
...
soap_wsse_encrypt_only(struct soap* soap, int alg, const void* key, int keylen, const char* tags)
{
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_encrypt_only", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
...
soap_wsse_encrypt(struct soap* soap, int alg, const void* key, int keylen)
{
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_encrypt", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
...
soap_wsse_decrypt_auto(struct soap* soap, int alg, const void* key, int keylen)
{
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_decrypt_auto", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
...
*/
int
soap_wsse_encrypt_begin(struct soap* soap, const char* id, int alg, const char* URI,
                        const char* keyname, const unsigned char* key)
{
	int event;
...
soap_wsse_decrypt_begin(struct soap* soap, const unsigned char* key)
{
	...
	if (!data)
	{
		return soap_set_receiver_error(soap, "soap_wsse_decrypt_begin", "Plugin not registered",
		                               SOAP_PLUGIN_ERROR);
	}
	...
	if (soap_in_ds__KeyInfoType(soap, "ds:KeyInfo", &info, NULL))
	{
		if (data->security_token_handler && info.KeyName)
		{
			...
			DBGLOG(TEST, SOAP_MESSAGE(fdebug,
			                          "Getting shared secret key '%s' through security_token_handler callback\n", info.KeyName));
	...
	if (!(alg & SOAP_MEC_ENV))
	{
		if (alg != data->deco_alg)
		{
			if (data->security_token_handler)
			{
				/* retrieve key from token handler callback */
				DBGLOG(TEST, SOAP_MESSAGE(fdebug,
				                          "Getting shared secret key for alg=%x through security_token_handler callback\n", alg));
...
soap_wsse_element_begin_out(struct soap* soap, const char* tag)
{
	...
	else if (data && data->encid && soap_tagsearch(data->encid, tag))
	{
		...
		return soap_wsse_encrypt_begin(soap, soap_wsse_ids(soap, tag), data->enco_alg, URI,
		                               data->enco_keyname, NULL);
	}
...
soap_wsse_preparesend(struct soap* soap, const char* buf, size_t len)
{
	...
	if (soap->event == SOAP_SEC_BEGIN)
	{
		...
		else if (!data->sigid || soap_tagsearch(data->sigid, soap->id))
		{
			...
			soap->event = SOAP_SEC_SIGN;
			digest = (struct soap_wsse_digest*)SOAP_MALLOC(soap,
			         sizeof(struct soap_wsse_digest) + strlen(soap->id) + 1);
...
soap_wsse_preparefinalsend(struct soap* soap)
{
	...
	if (data->digest)
	{
		...
		{
			return soap_set_receiver_error(soap, "wsse error",
			                               "Cannot use soap_wsse_sign with populated SignedInfo", SOAP_SSL_ERROR);
		}
		...
		if ((soap->mode & SOAP_IO) != SOAP_IO_CHUNK)
		{
			...
			DBGLOG(TEST, SOAP_MESSAGE(fdebug,
			                          "Counting the size of additional SOAP Header elements, mode=0x%x\n", soap->mode));
